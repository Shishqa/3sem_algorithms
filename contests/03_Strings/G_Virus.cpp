...
constexpr size_t MAX_STRING_LENGTH = 30001;
...
int main() {
    ...
    bool ans = vocabulary.has_cycle_inside();
...
bool Trie::has_cycle_inside() {
    ...
    while (!to_visit.empty()) {
        ...
        for (auto& node : target->go) {
            if (!node.second->is_terminal && get_up(node.second) == root) {
                if (!in_degree.count(node.second)) {
                    to_visit.push(node.second);
                    in_degree[node.second] = 0;
                    n_vertices++;
                }
                in_degree[node.second]++; 
            }
        }
    }
    ...
    while (!to_visit.empty()) {
        ...
        for (auto& go : target->go) {
            if (in_degree.count(go.second)) {
                int in_deg = --in_degree[go.second];
                if (!in_deg) {
                    to_visit.push(go.second);
                }
            }
        }
    ...
    return (n_vertices != visited_count);
...
void Trie::dump(const char* filename) {
    ...
    while (!to_visit.empty()) {
        ...
        for (auto& go_pair : target->go) {
            
            file << "\tnode" << target << " -> " << "node" << go_pair.second <<
                    "[color=\"purple\", label=\"" << go_pair.first << "\"];\n";
        ...
        if (target->short_suffix_link) {
            file << "\tnode" << target << " -> " << "node" << target->short_suffix_link <<
